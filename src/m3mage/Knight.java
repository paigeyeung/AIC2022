...
public class Knight extends AllyUnit {
...
//        }
        ...
        if(attackAndMoveToClosestEnemyOrNeutralOrShrine()) {
            uc.println("Knight chasing enemy");
            return;
        }
//        attackNearbyEnemyOrNeutralOrShrine();
        ...
        if(closestChest != null) {
            ...
            uc.println("Knight opening chest " + dest);
        ...
        if(action == 0) {
            uc.println("Knight moving to assembly " + assemblyLocation);
            dest = assemblyLocation;

            if(distanceSquaredToAssemblyLocation <= WITHIN_GROUP_MAX_DISTANCE) {
                uc.println("Knight joined assembly");
        ...
        if(action == 1) {
            uc.println("Knight waiting in assembly with " + lastNumTroopsInAssembly + " troops");
            dest = assemblyLocation;
            communication.addSelfToAssembly();

            if(lastNumTroopsInAssembly >= NUM_TROOPS_PER_ASSEMBLY) {
                uc.println("Knight assembly moving to group " + lastGroupCenterLocation);
        ...
        if(action == 3) {
            if(distanceSquaredToLastGroupCenterLocation > WITHIN_GROUP_MAX_DISTANCE) {
                uc.println("Knight rejoining group " + lastGroupCenterLocation);
                dest = lastGroupCenterLocation;
                action = 2;
            ...
            uc.println("Knight moving with group to " + groupAttackLocation);
    ...
    Direction getDirectionTo(Location destination) {
        ...
        for(Direction dir : directions) {
            if(uc.canMove(dir)) {
                ...
                uc.println("Knight: Cost of moving in direction " + dir + " is " + newF);
        ...
        if (movementDir != null) {
            uc.println("Knight moving in " + movementDir);
        }

        uc.println("Knight wants to move to destination " + destination + ", moves in dir " + movementDir + " to " + myLocation.add(movementDir));
        return movementDir;
    }
}