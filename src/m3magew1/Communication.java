...
//    final int INDEX_LOCATIONS = 1000;
    ...
    void determineAssemblyLocation() {
        ...
        int tileGridDimensions = (visionRangeTilesInOneDirection + 1) * 2 + 1;
        int[][] tileScores = new int[tileGridDimensions][tileGridDimensions];

        Location highestScoreLocation = null;
        int highestScore = -1;

        Location[] plainTileLocations = uc.senseVisibleTiles(TileType.PLAINS);
        for(Location tileLocation : plainTileLocations) {
            if(uc.isObstructed(selfLocation, tileLocation))
                continue;
            int tileX = tileLocation.x - selfLocation.x + tileGridDimensions / 2;
            int tileY = tileLocation.y - selfLocation.y + tileGridDimensions / 2;
            tileScores[tileX][tileY] = 20;
        }

        Location[] forestTileLocations = uc.senseVisibleTiles(TileType.FOREST);
        for(Location tileLocation : forestTileLocations) {
            if(uc.isObstructed(selfLocation, tileLocation))
                continue;
            int tileX = tileLocation.x - selfLocation.x + tileGridDimensions / 2;
            int tileY = tileLocation.y - selfLocation.y + tileGridDimensions / 2;
            tileScores[tileX][tileY] = 10;
        }
        ...
        for(Location tileLocation : plainTileLocations) {
            ...
            int selfDistanceSquared = selfLocation.distanceSquared(tileLocation);
            if(selfDistanceSquared >= 20)
                score += 70;
            else if(selfDistanceSquared >= 10)
                score += 50;
            ...
            if(highestScoreLocation == null || score > highestScore) {
                highestScoreLocation = tileLocation;
                highestScore = score;
            }
        }
