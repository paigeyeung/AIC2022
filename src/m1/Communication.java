...
//    final int INDEX_LOCATIONS = 1000;
    ...
    void checkForCornerUpdates() {
        if(cornerTrackingStatus == 0) {
            if(mapNorthBoundary != UNINITIALIZED_BOUNDARY && mapSouthBoundary != UNINITIALIZED_BOUNDARY
                    && mapWestBoundary != UNINITIALIZED_BOUNDARY && mapEastBoundary != UNINITIALIZED_BOUNDARY)
    ...
    void uploadEliminatedEnemyBaseCorner(Direction corner) {
        ...
        uc.writeOnSharedArray(INDEX_ENEMY_BASE_CORNERS, enemyBaseCorners);
        uc.println("Communication uploadEliminatedEnemyBaseCorner " + enemyBaseCorners + " (" + Integer.toBinaryString(enemyBaseCorners) + ")");
    }
    ...
    Location getEnemyBaseLocationIfEnemyBaseIsInCorner(Direction enemyBaseCorner) {
        ...
        if(enemyBaseCorner.isEqual(horizontalReflectionCorner)) {
            if(allyBaseCorner.isEqual(Direction.NORTHWEST) || allyBaseCorner.isEqual(Direction.NORTHEAST))
                return new Location(allyBaseLocation.x, mapSouthBoundary + (mapNorthBoundary - allyBaseLocation.y));
            return new Location(allyBaseLocation.x, mapNorthBoundary - (allyBaseLocation.y - mapSouthBoundary));
        }
        if(enemyBaseCorner.isEqual(verticalReflectionCorner)) {
            if(allyBaseCorner.isEqual(Direction.NORTHWEST) || allyBaseCorner.isEqual(Direction.SOUTHWEST))
                return new Location(mapEastBoundary - (allyBaseLocation.x - mapWestBoundary), allyBaseLocation.y);
            return new Location(mapWestBoundary + (mapEastBoundary - allyBaseLocation.x), allyBaseLocation.y);
        }
        if(enemyBaseCorner.isEqual(rotationalCorner)) {
            if(allyBaseCorner.isEqual(Direction.NORTHWEST))
                return new Location(mapEastBoundary - (allyBaseLocation.x - mapWestBoundary), mapSouthBoundary + (mapNorthBoundary - allyBaseLocation.y));
            if(allyBaseCorner.isEqual(Direction.NORTHEAST))
                return new Location(mapWestBoundary + (mapEastBoundary - allyBaseLocation.x), mapSouthBoundary + (mapNorthBoundary - allyBaseLocation.y));
            if(allyBaseCorner.isEqual(Direction.SOUTHWEST))
                return new Location(mapEastBoundary - (allyBaseLocation.x - mapWestBoundary), mapNorthBoundary - (allyBaseLocation.y - mapSouthBoundary));
            return new Location(mapWestBoundary + (mapEastBoundary - allyBaseLocation.x), mapNorthBoundary - (allyBaseLocation.y - mapSouthBoundary));
        }
        return null;
    }
    ...
    void lookForEnemyBase() {
        ...
        for(int i = 0; i < 4; i++) {
            if (((enemyBaseCorners >> i) & 1) == 1) {
                ...
                if(uc.canSenseLocation(expectedEnemyBaseLocation)) {
